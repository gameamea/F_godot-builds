...
if [ $result -eq 1 ]; then
  ...
  cmdScons $SCONS_FLAGS p=x11 tools=yes target=release_debug bits=64 mono_glue=no copy_mono_root=yes mono_prefix=$MONO_PREFIX_LINUX
  # Generate glue sources
  bin\godot.x11.opt.tools.32.mono --generate-mono-glue modules/mono/mono-installs/glue
  # Build binaries normally
  cmdScons $SCONS_FLAGS p=x11 tools=yes target=release_debug bits=64 mono_prefix=$MONO_PREFIX_LINUX
...
if [ $result -eq 1 ]; then
  cmdScons $SCONS_FLAGS p=x11 target=release_debug tools=no bits=64 mono_prefix=$MONO_PREFIX_LINUX
...
if [ $result -eq 1 ]; then
  cmdScons $SCONS_FLAGS p=windows tools=yes target=release_debug bits=64 copy_mono_root=yes mono_prefix=$MONO_PREFIX_WINDOWS
...
if [ $result -eq 1 ]; then
  cmdScons $SCONS_FLAGS platform=android target=release_debug android_arch=x86_64 mono_prefix=$MONO_PREFIX_ANDROID/android-x86_64-release
  cmdScons $SCONS_FLAGS platform=android target=release_debug android_arch=x86 mono_prefix=$MONO_PREFIX_ANDROID/android-x86-release
  cmdScons $SCONS_FLAGS platform=android target=release_debug android_arch=armv7 mono_prefix=$MONO_PREFIX_ANDROID/android-armeabi-v7a-debug
  cmdScons $SCONS_FLAGS platform=android target=release_debug android_arch=arm64v8 mono_prefix=$MONO_PREFIX_ANDROID/android-arm64-v8a-debug
