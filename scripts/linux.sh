...
if [ $buildLinuxEditor -eq 1 ]; then
  if [ $build32Bits -eq 1 ]; then
    if [ "$buildWithMono" -eq 1 ]; then
      ...
      # Generate the glue
      cmdScons platform=x11 bits=32 tools=yes target=release_debug mono_glue=no $LTO_FLAG $SCONS_FLAGS $MONO_FLAG $MONO_OPTIONS
      "$GODOT_DIR/bin/godot.x11.opt.tools.32${MONO_EXT}" --generate-mono-glue "$GODOT_DIR/modules/mono/glue"
      if [ $? -eq 0 ]; then result=1; else result=0; fi
      if [ $result -eq 1 ]; then echo_success "$label built successfully"; else echo_warning "$label built with error"; fi
      rm "$GODOT_DIR/bin/godot.x11.opt.tools.32${MONO_EXT}"
    fi
    # Build the editor
    ...
    rm -f $resultFile
    cmdScons platform=x11 bits=32 tools=yes target=release_debug $LTO_FLAG $SCONS_FLAGS $MONO_FLAG $MONO_OPTIONS
...
if [ $buildLinuxTemplates -eq 1 ]; then
  if [ $build32Bits -eq 1 ]; then
    if [ "$buildWithMono" -eq 1 ]; then
      ...
      rm -f $resultFile
      cmdScons platform=x11 bits=32 tools=no target=release_debug $LTO_FLAG $SCONS_FLAGS $MONO_FLAG $MONO_OPTIONS
      # Remove symbols and sections from files
      cmdUpxStrip $resultFile
      if [ $? -eq 0 ]; then result=1; else result=0; fi
      if [ $result -eq 1 ]; then echo_success "$label built successfully"; else echo_warning "$label built with error"; fi
    ...
    rm -f $resultFile
    cmdScons platform=x11 bits=32 tools=no target=release $LTO_FLAG $SCONS_FLAGS $MONO_FLAG $MONO_OPTIONS
    # Remove symbols and sections from files
    cmdUpxStrip $resultFile
...
if [ $buildLinuxEditor -eq 1 ]; then
  if [ "$buildWithMono" -eq 1 ]; then
    [ ! -z $MONO_PREFIX_LINUX ] && MONO_OPTIONS="$MONO_FLAG $MONO_PREFIX_LINUX/mono-installs/desktop-linux-x86_x64-release"
    # Generate the glue
    label="Generate the glue for 64 bits editor for Linux"
    echo_header "Running $label"
    cmdScons platform=x11 bits=64 tools=yes target=release_debug mono_glue=no $LTO_FLAG $SCONS_FLAGS $MONO_FLAG $MONO_OPTIONS
    "$GODOT_DIR/bin/godot.x11.opt.tools.64${MONO_EXT}" --generate-mono-glue "$GODOT_DIR/modules/mono/glue"
    if [ $? -eq 0 ]; then result=1; else result=0; fi
    if [ $result -eq 1 ]; then echo_success "$label built successfully"; else echo_warning "$label built with error"; fi
    rm "$GODOT_DIR/bin/godot.x11.opt.tools.64${MONO_EXT}"
  fi
  ...
  rm -f $resultFile
  cmdScons platform=x11 bits=64 tools=yes target=release_debug $LTO_FLAG $SCONS_FLAGS $MONO_FLAG $MONO_OPTIONS
  # Remove symbols and sections from files
  cmdUpxStrip $resultFile
  if [ $? -eq 0 ]; then result=1; else result=0; fi # line just for easier comparison with windows.h
  if [ $result -eq 1 ]; then echo_success "$label built successfully"; else echo_warning "$label built with error"; fi
...
if [ $buildLinuxTemplates -eq 1 ]; then
  if [ "$buildWithMono" -eq 1 ]; then
    ...
    echo_header "Running $label"
    [ ! -z $MONO_PREFIX_LINUX ] && MONO_OPTIONS="$MONO_FLAG $MONO_PREFIX_LINUX/mono-installs/desktop-linux-x86_x64-debug"
    resultFile="$GODOT_DIR/bin/godot.x11.opt.debug.64${MONO_EXT}"
    rm -f $resultFile
    cmdScons platform=x11 bits=64 tools=no target=release_debug $LTO_FLAG $SCONS_FLAGS $MONO_FLAG $MONO_OPTIONS
    # Remove symbols and sections from files
    cmdUpxStrip $resultFile
    if [ $? -eq 0 ]; then result=1; else result=0; fi # line just for easier comparison with windows.h
    if [ $result -eq 1 ]; then echo_success "$label built successfully"; else echo_warning "$label built with error"; fi
  ...
  rm -f $resultFile
  cmdScons platform=x11 bits=64 tools=no target=release $LTO_FLAG $SCONS_FLAGS $MONO_FLAG $MONO_OPTIONS
  # Remove symbols and sections from files
  cmdUpxStrip $resultFile
  if [ $? -eq 0 ]; then result=1; else result=0; fi # line just for easier comparison with windows.h
  if [ $result -eq 1 ]; then echo_success "$label built successfully"; else echo_warning "$label built with error"; fi
